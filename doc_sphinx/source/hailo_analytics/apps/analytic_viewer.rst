...
The ``app-analytic-draw-client.py`` is a Python-based video player that runs on the host machine and is designed to synchronize real-time video streams with external analytic metadata. It receives H.264 video over UDP, extracts SEI (Supplemental Enhancement Information) NAL units for frame-accurate timing, and synchronizes this with object detection data received via ZeroMQ (ZMQ). 

The application uses **Cairo** to draw bounding boxes and labels directly onto the video frames.
...
* **ZMQ Synchronization**: Matches H15 analytics metadata with specific video frames using isp timestamps.
...
* ``-p, --port PORT``: UDP port to receive video stream (default: 5000)
...
.. code-block:: bash
    ...
    python app_analytic_draw_client.py --port 5005
...
.. code-block:: bash
    ...
    python app_analytic_draw_client.py --port 5005 --analytic-data-ip 10.0.0.3 --face-landmark-filter 1 --debug-perf

Technical Logic
===============

1. **ZMQ Listener**: Runs in a background thread, collecting metadata into a queue.
2. **SEI Probe**: A GStreamer probe monitors the incoming H.264 stream for SEI units containing timestamps.
3. **Synchronization**: When a timestamp is found in the video, the script searches the ZMQ queue for a matching entry. If found, it is added to a draw registry with the frame PTS as key.
