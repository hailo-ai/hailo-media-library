...
**Smart ROI Encoder**
   ...
   The Smart ROI Encoder feature enables efficient bitrate allocation by concentrating quality in user-defined
   Regions of Interest (ROIs), while allowing the encoder to compress background regions more aggressively.

   **Compatibility:** The Smart ROI Encoder is supported in CVBR mode and H264 only and works with
   the Camera Viewer web UI, where users can draw bounding boxes that will be prioritized, 
   or directly in the encoder json configuration under ``smart_encoder`` section.
