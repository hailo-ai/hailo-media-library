...
- **Semantic Segmentation Analytics**: Stores semantic segmentation results with per-class masks and detection data


.. note::
  For an example of the Analytics Database in use, refer to Dynamic Privacy Masks in :ref:`Privacy Mask <privacy-mask-label>`. and to the `dynamic_privacy_mask` reference application, which demonstrate how to integrate semantic segmentation results for dynamic privacy masking.
...
The Analytics Database can be configured using the `application_analytics` section in the profile configuration file, and is used to initalize the Database during the Media Library initialization. The configuration supports the two types of analytics data: `semantic_segmentation` and `detection`. Both are optional.
...
Semantic segmentation entries additionally require:

- **mask_size**: The size in bytes of a single mask payload.
...
.. code-block:: cpp
    ...
    semantic_segmentation_analytics_config_t seg_config;
    ...
    seg_config.original_width_ratio = 4;
    seg_config.original_height_ratio = 3;
    seg_config.max_entries = 5;
    seg_config.mask_size = 307200;
    seg_config.labels = {{"person", 0}};

    config.semantic_segmentation_analytics_config["segmentation_results"] = seg_config;
...
.. code-block:: cpp
    ...
    SemanticSegmentationAnalyticsData seg_data;
    seg_data.ts = frame_media_lib_buffer.isp_timestamp_ns;
    seg_data.analytics_buffer = {/* segmentation results with masks */};
    seg_data.medialib_buffer_ptrs = {/* tensor buffers holding the mask memory */};

    auto result = analytics_db.add_semantic_segmentation_entry("segmentation_results", seg_data);
    if (result != MEDIA_LIBRARY_SUCCESS) {
        // Handle error
    }

.. note::
   The `analytics_buffer` in both cases should contain the serialized data of the detection or segmentation results, which is a vector of hailort structs. For detection, those structs are of type ``hailo_detection_t`` and for semantic segmentation, they are of type ``hailo_semantic_segmentation_mask_t``. The buffer used for the timestamp is the original buffer holding the frame, as opposed to `medialib_buffer_ptrs` in the semantic segmentation data, which is used to reference the Media Library buffers that contain the mask memory allocated for the AI inference results.
...
2. **Closest Timestamp Query**:
   ...
   .. code-block:: cpp
      ...
      auto result = analytics_db.query_semantic_segmentation_entry("segmentation_results", options);
