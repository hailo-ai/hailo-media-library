...
{
  private:
    // Guards m_buffer: the same Buffer instance is fanned out to multiple stage threads, which
    // read it via get_buffer() while another stage may swap it in release_frame_data().
  ...
  public:
    ...
    /**
     * @brief Release the underlying DMA frame data while preserving metadata accessors.
     *
     * Replaces the internal HailoMediaLibraryBufferPtr with a lightweight shadow that keeps
     * width/height/format and timestamps (isp_timestamp_ns, pts, dts, duration) reachable via
     * get_buffer() but holds no DMA plane data. The underlying frame buffer is returned to its
     * pool while this Buffer continues to flow downstream as a metadata-only carrier.
     *
     * Caller must ensure no downstream consumer reads pixel data from this Buffer after the
     * call; doing so will yield empty planes.
     */
