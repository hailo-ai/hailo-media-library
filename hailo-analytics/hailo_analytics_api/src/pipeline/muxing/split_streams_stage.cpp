...
AppStatus SplitStreamsStage::process(BufferPtr buffer)
{
    ...
    // Optional: share the carrier's HailoROI (the AI postprocess tree built by the AI pipeline)
    // onto each passenger's analytics Buffer. Shallow shared_ptr — every stream's downstream
    // stages walk the same tree to produce per-stream wire-types at their own boundaries.
    if (m_propagate_roi)
    {
        ...
        if (carrier_roi)
        {
            ...
            {
                auto p_buf = md->get_buffer();
                if (p_buf)
                    p_buf->set_roi(carrier_roi);
            }
        }
    }
