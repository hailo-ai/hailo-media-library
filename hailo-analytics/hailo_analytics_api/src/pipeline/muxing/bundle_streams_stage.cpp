...
void BundleStreamsStage::loop()
{
    ...
    {
        BufferPtr carrier = m_carrier_queue->pop();
        if (!carrier)
        {
            HAILO_ANALYTICS_LOG_DEBUG("BundleStreamsStage[{}]: carrier queue closed — flushing", m_stage_name);
            break;
        }
        m_tracing->trace_processing_start(carrier);

        // Block-pop one passenger per declared stream, in roster order. Direct queue access via
        // cached members — no per-frame name lookup.
        bool all_present = true;
        std::vector<BufferPtr> passengers;
        passengers.reserve(m_passenger_queues.size());
        for (size_t i = 0; i < m_passenger_queues.size(); ++i)
        {
            BufferPtr passenger = m_passenger_queues[i]->pop();
            if (!passenger)
            {
                HAILO_ANALYTICS_LOG_WARN(
                    "BundleStreamsStage[{}]: passenger queue '{}' closed during bundle assembly — flushing",
