...
void HailortAsyncStage::set_scheduler_threshold(int threshold)
{
    if (threshold == m_scheduler_threshold)
    {
        return;
    }
    HAILO_ANALYTICS_LOG_INFO("[{}] updating scheduler threshold from {} to {}", m_stage_name, m_scheduler_threshold,
                             threshold);
    m_scheduler_threshold = threshold;
    auto status = m_configured_infer_model.set_scheduler_threshold(threshold);
    if (status != HAILO_SUCCESS)
    {
        HAILO_ANALYTICS_LOG_ERROR("[{}] Failed to set scheduler threshold to {}, status = {}", m_stage_name, threshold,
                                  status);
    }
}
