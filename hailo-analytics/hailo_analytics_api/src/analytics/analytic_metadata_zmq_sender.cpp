...
zeromq_config_t base_zeromq_sender_config()
{
    ...
    config.pub_address = "tcp://" + get_local_ip_address() + ":7000";
...
generate_analytic_metadata_zmq_sender_pipeline(const std::string &pipeline_name,
                                               std::optional<analytic_metadata_zmq_sender_config_t> user_configs)
{
    analytic_metadata_zmq_sender_config_t cfg = base_analytic_metadata_zmq_sender_config(); // default configs
