...
TEST_F(DetectionTrackerStageTest, ProcessWithDetections)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        add_detection_to_roi(buffer->get_roi(), 100.0f, 100.0f, 50.0f, 50.0f, 0, "person", 0.9f);
        EXPECT_EQ(stage->process(buffer), AppStatus::SUCCESS);
    }
    ...
    add_detection_to_roi(check_buffer->get_roi(), 100.0f, 100.0f, 50.0f, 50.0f, 0, "person", 0.9f);
...
TEST_F(DetectionTrackerStageTest, MotOutputCreatesFile)
{
    ...
    // Feed detections to produce tracked output
    for (int i = 0; i < 3; i++)
    {
        BufferPtr buffer = create_test_buffer_with_tensor_metadata();
        add_detection_to_roi(buffer->get_roi(), 100.0f, 200.0f, 50.0f, 60.0f, 0, "person", 0.85f);
        EXPECT_EQ(stage->process(buffer), AppStatus::SUCCESS);
    }
    ...
    std::ifstream mot_file(MOT_TEST_FILE);
...
TEST_F(DetectionTrackerStageTest, MotOutputFormat)
{
    ...
    {
        BufferPtr buffer = create_test_buffer_with_tensor_metadata();
        add_detection_to_roi(buffer->get_roi(), 10.0f, 20.0f, 30.0f, 40.0f, 0, "person", 0.95f);
        EXPECT_EQ(stage->process(buffer), AppStatus::SUCCESS);
    }
...
TEST_F(DetectionTrackerStageTest, TrackletsToHailoDetections)
{
    ...
    // Build tracklets manually
    hailo_tracklet_t tracklet_arr[2];
    tracklet_arr[0].id = 42;
    tracklet_arr[0].detection = {20.0f, 10.0f, 80.0f, 60.0f, 0.9f, 0};
    ...
    tracklet_arr[1].detection = {200.0f, 100.0f, 250.0f, 150.0f, 0.85f, 1};
    ...
    EXPECT_FLOAT_EQ(bbox0.xmin(), 10.0f);
    EXPECT_FLOAT_EQ(bbox0.ymin(), 20.0f);
    EXPECT_FLOAT_EQ(bbox0.width(), 50.0f);  // 60 - 10
    EXPECT_FLOAT_EQ(bbox0.height(), 60.0f); // 80 - 20
...
TEST_F(DetectionTrackerStageTest, TrackletsToHailoDetectionsFiltersNonTracked)
{
    ...
    tracklet_arr[0].id = 1;
    tracklet_arr[0].detection = {10.0f, 20.0f, 30.0f, 40.0f, 0.9f, 0};
    ...
    tracklet_arr[1].id = 2;
    tracklet_arr[1].detection = {50.0f, 60.0f, 70.0f, 80.0f, 0.85f, 0};
    tracklet_arr[1].state = HAILO_TRACKLET_STATE_TRACKED;
    tracklet_arr[1].frames_since_update = 0;
    ...
    tracklet_arr[2].id = 3;
    tracklet_arr[2].detection = {90.0f, 100.0f, 110.0f, 120.0f, 0.7f, 0};
...
TEST_F(DetectionTrackerStageTest, TrackletsToHailoDetectionsUnknownLabel)
{
    ...
    tracklet_arr[0].id = 7;
    tracklet_arr[0].detection = {10.0f, 20.0f, 30.0f, 40.0f, 0.9f, 5}; // class_id 5 not in labels
    tracklet_arr[0].state = HAILO_TRACKLET_STATE_TRACKED;
    tracklet_arr[0].frames_since_update = 0;
    tracklet_arr[0].total_frames_tracked = 1;
    tracklet_arr[0].velocity_x = 0.0f;
