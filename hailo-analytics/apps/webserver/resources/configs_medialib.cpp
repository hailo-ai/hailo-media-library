...
{
    // Canonicalize so relative refs (e.g. ../profiles/...) anchor to the real arch-specific dir
    // rather than the /etc/imaging/cfg/medialib_configs symlink (which would resolve ".." wrong).
    std::error_code ec;
    auto canonical = std::filesystem::weakly_canonical(config_path, ec);
    m_medialib_config_path = ec ? config_path : canonical.string();
...
{
    const std::string medialib_dir = std::filesystem::path(m_medialib_config_path).parent_path().string();
    for (auto profile : m_medialib_config["profiles"])
    {
        if (profile["name"] == profile_name)
        {
            const std::string resolved_path = resolve_relative_to(profile["config_file"], medialib_dir);
            m_current_profile_path = resolved_path;
            return load_config_from_file(resolved_path);
        }
    }
...
tl::expected<nlohmann::json, std::string> ConfigResourceMedialib::enable_gyro_if_exist(nlohmann::json profile)
{
    try
    {
        ...
        {
            const std::string profile_dir = std::filesystem::path(m_current_profile_path).parent_path().string();
            const std::string stabilizer_config_path = resolve_relative_to(profile["stabilizer_settings"], profile_dir);
...
std::string ConfigResourceMedialib::get_current_profile_path() const
{
    return m_current_profile_path;
}
