...
void FaceLandmarksPipeline::build_pipeline()
{
    ...
    m_landmarks_crop = std::dynamic_pointer_cast<hailo_analytics::pipeline::cropping::BBoxCropStage>(
        landmarks_pipeline->get_stage_by_name(
            std::string(hailo_analytics::analytics::face_landmarks::BBOX_CROP_STAGE)));
    if (!m_landmarks_crop)
    {
        WEBSERVER_LOG_ERROR("Failed to recover landmarks bbox crop stage by name");
        throw std::runtime_error("Failed to recover landmarks bbox crop stage");
    }
...
void FaceLandmarksPipeline::callback_handle_profile_switch(ResourceStateChangeNotification notif)
{
    // Base call applies the new profile to the media library; re-apply the crop budget and queue
    // mode afterwards so they reflect the now-active profile.
    ...
    if (!current_profile.has_value())
    {
        WEBSERVER_LOG_ERROR("Failed to get current profile while updating landmarks config");
        return;
    }

    const size_t new_max_crops = face_landmarks_app::get_max_crops(current_profile.value());
    m_landmarks_crop->set_max_crops(new_max_crops);

    // Leak the detection->landmarks boundary queue only on h15h+HDR (see default_landmarks_config).
    const bool leaky = face_landmarks_app::is_hailo15h_hdr(current_profile.value());
    m_landmarks_crop->set_leaky(leaky);

    WEBSERVER_LOG_INFO("Face landmarks max_crops updated to {} (leaky={}) for profile {}", new_max_crops, leaky,
                       current_profile.value().name);
}
