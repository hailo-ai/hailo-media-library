...
static const std::vector<HailoBBox> SINGLE_TILE_LAYOUT = {{0.0, 0.0, 1.0, 1.0}};
...
void DetectionPipeline::build_pipeline()
{
    ...
    if (single_tile)
    {
        tiling_cfg.tiling_config.bbox_tiles = SINGLE_TILE_LAYOUT;
        ...
        tiling_cfg.aggregator_config.multi_scale = false;
    }
    else
    {
...
{
    const auto sensor_type = webserver::common::get_sensor_type();
    const bool sensor_match = sensor_type == webserver::common::SensorModel::SENSOR_IMX678 ||
                              sensor_type == webserver::common::SensorModel::SENSOR_IMX675 ||
                              sensor_type == webserver::common::SensorModel::SENSOR_IMX662;
    const bool profile_match =
        m_current_profile_type == ProfileType::AiIspGen3 || m_current_profile_type == ProfileType::AiIspGen3_1;
    return m_app_resources->platform == Architecture::Hailo15L && sensor_match && profile_match;
}
...
void DetectionPipeline::callback_handle_profile_switch(ResourceStateChangeNotification notif)
{
    ...
    const bool single_tile = is_single_tile_mode();
    const size_t crop_every_x_frames = get_crop_every_x_frames();
    ...
    if (!ai_pipeline)
    {
        WEBSERVER_LOG_ERROR("Failed to get AI sub-pipeline on profile switch");
        return;
    }
    ...
    if (!tiling_stage)
    {
        WEBSERVER_LOG_ERROR("Failed to get tiling stage on profile switch");
        return;
    }

    tiling_stage->set_crop_every_x_frames(crop_every_x_frames);
    tiling_stage->set_bbox_tiles(single_tile ? SINGLE_TILE_LAYOUT : tiling::DEFAULT_TILES);

    auto aggregator_stage = std::dynamic_pointer_cast<AggregatorStage>(
        ai_pipeline->get_stage_by_name(std::string(tiling::TILING_AGGREGATOR_STAGE)));
    if (aggregator_stage)
    {
        aggregator_stage->set_multiscale(!single_tile);
    }
    else
    {
        WEBSERVER_LOG_WARN("Failed to get tiling aggregator stage on profile switch; multiscale flag not updated");
    }

    // The detector's HailoRT scheduler threshold is fixed at build time from the tile count, so it must
    // also follow the runtime tile-layout change: a stale multi-tile threshold makes the scheduler wait out
    // its timeout on every single-tile frame, adding stream latency without dropping FPS.
    const int scheduler_threshold =
        static_cast<int>(single_tile ? SINGLE_TILE_LAYOUT.size() : tiling::DEFAULT_TILES.size());
    auto detection_subpipeline = std::dynamic_pointer_cast<hailo_analytics::pipeline::Pipeline>(
        ai_pipeline->get_stage_by_name(std::string(tiling::DETECTION_SUBPIPELINE)));
    auto detection_stage = detection_subpipeline
                               ? std::dynamic_pointer_cast<HailortAsyncStage>(
                                     detection_subpipeline->get_stage_by_name(std::string(detection::DETECTION_STAGE)))
                               : nullptr;
    if (detection_stage)
    {
        detection_stage->set_scheduler_threshold(scheduler_threshold);
    }
    else
    {
        WEBSERVER_LOG_WARN("Failed to get detection stage on profile switch; scheduler threshold not updated");
    }

    WEBSERVER_LOG_INFO("Profile switch: single_tile={}, tiles={}, crop_every_x_frames={}", single_tile,
                       scheduler_threshold, crop_every_x_frames);
}
