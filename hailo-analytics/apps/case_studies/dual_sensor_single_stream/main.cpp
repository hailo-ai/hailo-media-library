...
void configure_media_library(std::shared_ptr<AppResources> app_resources)
{
    for (int i = 0; i < AppResources::NUM_SENSORS; ++i)
    {
        std::string medialib_config_string =
            read_string_from_file(app_resources->instances[i].medialib_config_path.c_str());
        ...
        if (app_resources->instances[i].media_library->initialize(medialib_config_string) !=
