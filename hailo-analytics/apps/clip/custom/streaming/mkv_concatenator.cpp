...
bool MkvConcatenator::start(const std::vector<std::string> &file_paths)
{
    ...
    // Bound the appsink queue (max-buffers=4, drop=FALSE) so the chain function blocks when full
    // and HTTP back-pressure propagates upstream → matroskamux → parser → concat → queues → demux → filesrc.
    static constexpr guint APPSINK_MAX_BUFFERS = 4;
    g_object_set(G_OBJECT(m_appsink), "emit-signals", FALSE, "sync", FALSE, "max-buffers", APPSINK_MAX_BUFFERS, "drop",
                 FALSE, nullptr);
    ...
    for (size_t i = 0; i < file_paths.size(); ++i)
    {
        ...
        // Cap each per-file queue. Prevent huge memory allocation when
        // reading large amount of file which could lead to 100+MB
        static constexpr guint QUEUE_MAX_BUFFERS = 50;
        static constexpr guint QUEUE_MAX_BYTES = 1 * 1024 * 1024;
        static constexpr guint64 QUEUE_MAX_TIME_NS = 500 * GST_MSECOND;
        g_object_set(G_OBJECT(queue), "max-size-buffers", QUEUE_MAX_BUFFERS, "max-size-bytes", QUEUE_MAX_BYTES,
                     "max-size-time", QUEUE_MAX_TIME_NS, nullptr);
...
void MkvConcatenator::cleanup()
{
    if (m_pipeline)
    {
        // Drain any samples still queued in the appsink before tearing down the pipeline.
        // Closes a race where a sample was pushed to the appsink's internal queue but never
        // pulled (e.g. HTTP client disconnected mid-chunk) — without this it can leak per call.
        if (m_appsink)
        {
            static constexpr auto MAX_DRAIN_DURATION = std::chrono::milliseconds(100);
            const auto drain_deadline = std::chrono::steady_clock::now() + MAX_DRAIN_DURATION;
            GstSample *pending_sample;
            while ((pending_sample = gst_app_sink_try_pull_sample(GST_APP_SINK(m_appsink), 0)) != nullptr)
            {
        ...
        // Release requested concat pads (release_request_pad + unref, per GStreamer convention)
        for (auto &ctx : m_demux_contexts)
        {
            if (ctx.concat_sink_pad)
            {
                GstElement *concat_element = gst_pad_get_parent_element(ctx.concat_sink_pad);
                if (concat_element)
                {
                    gst_element_release_request_pad(concat_element, ctx.concat_sink_pad);
                    gst_object_unref(concat_element);
                }
        ...
        // Return freed heap chunks to the OS so RSS does not stay elevated across invocations.
        malloc_trim(0);
    }
