...
class ClipQueryServiceExt : public hailo_analytics::analytics::app_constructor::CameraAppExtension
{
  public:
        ...
        QueryEmbeddingInfo &query_data)
    {
        ...
        for (const auto &neg : query_data.m_negative_embeddings)
        {
            negative_prompts.push_back(neg.prompt);
        }
  ...
  private:
        ...
        std::vector<PartitionedFaissDB::SearchResult> &faiss_index_search_result, QueryEmbeddingInfo &query_data)
    {
        ...
        for (const auto &thumbnail_result : thumbnail_table_results)
        {
            ...
            if (faiss_table_it != faiss_table_results.end())
            {
                ...
                auto faiss_search_it = std::find_if(faiss_index_search_result.begin(), faiss_index_search_result.end(),
                                                    [&faiss_table_it](const auto &faiss_result) {
                                                        return faiss_result.id == faiss_table_it->faiss_id;
                                                    });
    ...
    std::vector<PositiveQueryFullRecord> filter_with_negative_embeddings(
        std::vector<PositiveQueryFullRecord> records, // Passed by value to work on a copy
        std::vector<QueryEmbeddingInfo::EmbeddingData> &negative_embeddings, float confidence_threshold = 0.8f)
    {
        ...
        for (const auto &record : records)
        {
            ...
            // Calculate cosine similarity with each negative embedding
            std::vector<float> scores{record.m_score};
            for (const auto &negative_embedding : negative_embeddings)
            {
                calculate_similarities_for_normalized(record.m_embedding, negative_embedding.embedding, scores);
            }
            ...
            /*
            std::cout << "Scores for prompt: " << record.m_prompt << ": Positive Score = " << scores[0] << ",
                Negative Scores = ";
     ...
     */
    std::vector<PositiveQueryFullRecord> filter_and_sort_by_score(
        std::vector<PositiveQueryFullRecord> records, // Passed by value to work on a copy
