  ...
  public:
    ...
    bool clean_up(StorageCleanupServiceExt &cleanup_service) override
    {
        ...
        for (const auto &network_name : faiss_index_names)
        {
            ...
            // The file names list is already ordered from oldest to newest, we start removing the oldest file base on
            // the cleanup_param_float percentage
            // AARON TODO:  Instead of this simple logic we could retrieve all index per shard file, calculate total
            //              and then determine (base on cleanup_param_float to calculate total index to remove
            //              converting number of file) how many files to remove per shard. This is useful for the case
            //              when object refresh rate dynamically changes at runtime which could impact the number of
            //              files to remove with imbalanced total index to be removed as whole percentage.
            size_t num_files_to_remove =
                static_cast<size_t>(std::ceil(file_names.size() * cleanup_param_float / 100.0f));
            ...
            for (size_t i = 0; i < num_files_to_remove; ++i)
            {
                ...
                auto thumbnails =
                    cleanup_service.get_thumbnail_table()->get_records_by_timestamp_before(latest_timestamp);
