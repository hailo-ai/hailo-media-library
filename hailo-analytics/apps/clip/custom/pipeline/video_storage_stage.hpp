...
class VideoStorageStage : public hailo_analytics::pipeline::ThreadedStage
{
  ...
  public:
    inline VideoStorageStage(std::string name, DBSource db_source, std::string db_source_data, std::string video_dir,
                             std::string video_filename_prefix,
                             uint32_t video_segment_duration_seconds = VIDEO_DURATION_SECONDS_DEFAULT,
                             bool enable = true, size_t queue_size = VIDEO_STORAGE_QUEUE_SIZE_DEFAULT,
        ...
        : hailo_analytics::pipeline::ThreadedStage(name, queue_size, leaky, trace_processing_operations),
          ...
          m_video_filename_prefix(video_filename_prefix),
          m_video_segment_duration_seconds(video_segment_duration_seconds), m_enable(enable), m_should_terminate(false)
    {
    }
  ...
  private:
                                     ...
                                     [[maybe_unused]] uint32_t segment_index, void *user_data)
    {
        ...
        // We keep the recorded video file if there is event, otherwise we delete the file
        if (video_storage_stage->m_event_captured.load())
        {
                                   ...
                                   uint32_t segment_index)
    {
        ...
        std::cout << "Duration: " << duration_ms << " ms" << "(" << duration_ms / 1000.0 << " seconds)" << std::endl;
    ...
    void database_access()
    {
        ...
        while (!m_should_terminate.load())
        {
            ...
            if (!m_video_cache_dir.empty())
            {
                ...
                if (duration.count() > 30)
                {
                    HAILO_ANALYTICS_LOG_INFO("Time taken Video files move from cache to storage: {} ms",
                                             duration.count());
                }
            }
      ...
      public:
        ...
        std::shared_ptr<VideoStorageStage> buildptr() const
        {
            ...
            return std::make_shared<VideoStorageStage>(m_stage_name.value(), m_db_source, m_db_source_data.value(),
                                                       ...
                                                       m_video_segment_duration_seconds, m_enable, m_queue_size,
                                                       m_leaky, m_trace);
        }
    };
