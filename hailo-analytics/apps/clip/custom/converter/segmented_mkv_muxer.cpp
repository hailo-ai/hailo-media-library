...
bool GStreamerMkvSegmenter::create_pipeline()
{
    ...
    // Add flow-control limits — sized to absorb possible eMMC stalls caused by
    // concurrent playback reads contending with segment-write I/O.
    const guint64 maxBytes = 24 * 1024 * 1024;      // 24 MB minimum
    const guint maxBuffers = 180;                   // 180 frames minimum
