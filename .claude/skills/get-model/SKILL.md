---
name: get-model
description: Propose and download a HEF from the public Hailo Model Zoo — either a swap for the model currently in a pipeline, or a fresh pick for a new pipeline. Use when the user says "find me a model for X", "what can I use instead of yolov8s", "swap to something more accurate / faster / smaller", or "I want to add a face-recognition stage". Picks per H15L/H15H, explains the accuracy↔speed tradeoff, and curls the HEF.
...
2. **Fetch the table.** `WebFetch` the page and extract the model rows: Network Name, Float mAP, Hardware mAP, FPS (B1), FPS (B8), Input Resolution, Params (M), OPS (G).

3. **Pick 2–3 candidates** for the user's preference. Bias by this order, because it minimises downstream work:
   ...
   - **Newer architecture, same task** (secondary): `yolov8s → yolov11s` / `yolov26s`. Often a better accuracy/OPS curve, but postprocess function and JSON may differ — flag that to the user.
...
4. **Present the shortlist.** For each candidate, one line: name • mAP • FPS@B1 • OPS(G) • Params(M) • one-sentence note ("same family, ~2× faster, ~1.5pt mAP drop"). Wait for the user to pick.

5. **Download the HEF.** Public S3 URL pattern:
   ```
...
- *"Swap detection from yolov8s to something faster on H15L"* → shortlist `yolov8n` (same family, ~2× FPS, ~7pt mAP drop) and `yolov6n` (different family, similar profile). Download `yolov8n.hef`, hand off to `/swap-model`.
- *"Add face recognition to the face-landmarks pipeline on H15L"* → fetch `HAILO15L_face_recognition.rst`, shortlist `arcface_mobilefacenet` (low OPS, fast) and `arcface_r50` (more accurate, heavier). Download chosen HEF, hand off to `/edit-pipeline`.
