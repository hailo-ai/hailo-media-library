...
template <typename T> class ConcurrentQueue
{
  public:
    ...
    void enqueue_many(const std::vector<T> &items)
    {
        ...
        if (m_size + items.size() > m_capacity)
        {
            // If adding these items exceeds capacity, we need to drop the oldest items
            size_t excess = (m_size + items.size()) - m_capacity;
            m_head = (m_head + excess) % m_capacity; // Move head forward by the number of excess items
            m_size = m_capacity;
        }
        else
        {
                                ...
                                const std::function<bool(const T &)> &continue_predicate = nullptr)
    {
        ...
        while (count > 0)
        {
            ...
            current_head = (current_head + 1) % m_capacity; // Move forward
            --count;
        }
    ...
    {
        std::unique_lock<std::mutex> lock(m_mutex);
        size_t current_tail = (m_tail + m_capacity - 1) % m_capacity; // Start from the last item
        size_t count = m_size;
        while (count > 0)
        {
            ...
            current_tail = (current_tail + m_capacity - 1) % m_capacity; // Move backward
            --count;
        }
        return std::nullopt; // No matching item found
    }
