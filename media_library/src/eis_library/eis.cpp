                      ...
                      double iir_hpf_coefficient)
{
    ...
    if (prev_high_pass.prev_gyro_x == 0.0 && prev_high_pass.prev_gyro_y == 0.0 && prev_high_pass.prev_gyro_z == 0.0)
    {
        // Initialize the previous high-pass values with the first gyro sample
        prev_high_pass.prev_gyro_x = static_cast<double>(gyro_records[0].vx) * gyro_scale;
        prev_high_pass.prev_gyro_y = static_cast<double>(gyro_records[0].vy) * gyro_scale;
        prev_high_pass.prev_gyro_z = static_cast<double>(gyro_records[0].vz) * gyro_scale;
    }
    ...
    {
        double corrected_vx = static_cast<double>(gyro.vx) * gyro_scale;
        double corrected_vy = static_cast<double>(gyro.vy) * gyro_scale;
        double corrected_vz = static_cast<double>(gyro.vz) * gyro_scale;
...
void EIS::reset_history(bool reset_iir_convergence, bool reset_prev_high_pass)
{
    ...
    if (reset_iir_convergence)
    {
        m_iir_convergence_count = IIR_CONVERGENCE_COUNT;
    }

    if (reset_prev_high_pass)
    {
        prev_high_pass = {0, 0, 0, 0, 0, 0};
    }
}
