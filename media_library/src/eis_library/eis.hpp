...
/* Define The ALTERNATIVE_ALGORITHM macro to use the alternative algorithm */
#define ALTERNATIVE_ALGORITHM
...
#ifdef ALTERNATIVE_ALGORITHM
    std::vector<std::pair<uint64_t, cv::Mat>> integrate_rotations_rolling_shutter(const std::vector<unbiased_gyro_sample_t> &frame_gyro_records);
#else
