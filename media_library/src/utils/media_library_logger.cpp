...
#define MEDIALIB_CONSOLE_LOGGER_PATTERN ("[%Y-%m-%d %X.%e] [%P] [%t] [%n] [%^%l%$] [%s:%#] [%!] %v") // Console logger will print: [timestamp] [PID] [TID] [MediaLib] [log level] [source file:line number] [function name] msg
#define MEDIALIB_MAIN_FILE_LOGGER_PATTERN ("[%Y-%m-%d %X.%e] [%P] [%t] [%n] [%l] [%s:%#] [%!] %v")   // File logger will print: [timestamp] [PID] [TID] [MediaLib] [log level] [source file:line number] [function name] msg
#define MEDIALIB_LOCAL_FILE_LOGGER_PATTERN ("[%Y-%m-%d %X.%e] [%t] [%n] [%l] [%s:%#] [%!] %v")       // File logger will print: [timestamp] [TID] [MediaLib] [log level] [source file:line number] [function name] msg

#define MEDIALIB_LOGGER_PATH_ENV_VAR ("MEDIALIB_LOGGER_PATH")
#define MEDIALIB_LOGGER_FLUSH_EVERY_PRINT_ENV_VAR ("MEDIALIB_LOGGER_FLUSH_EVERY_PRINT")
...
std::string MediaLibLoggerSetup::get_main_log_path()
{
    ...
    const auto full_path = hailo_dir_path + std::string(PATH_SEPARATOR) + std::string(MEDIALIB_NAME);
    bool success;
...
std::shared_ptr<spdlog::sinks::sink> MediaLibLoggerSetup::create_file_sink(const std::string &dir_path, const std::string &filename, bool rotate)
{
    if ("" == dir_path)
    {
    ...
    if (!is_dir)
    {
        std::cerr << "MediaLib warning: Cannot create log file " << filename << "! Path " << dir_path << " is not valid." << std::endl;
        return make_shared_nothrow<spdlog::sinks::null_sink_st>();
    }

    if (!std::filesystem::exists(dir_path))
    {
        std::cerr << "MediaLib warning: Cannot create log file " << filename << "! Please check the directory " << dir_path << " write permissions." << std::endl;
        return make_shared_nothrow<spdlog::sinks::null_sink_st>();
    }
    ...
    if (rotate)
    {
        return make_shared_nothrow<spdlog::sinks::rotating_file_sink_mt>(file_path, MAX_LOG_FILE_SIZE, MEDIALIB_MAX_NUMBER_OF_LOG_FILES);
    }

    return make_shared_nothrow<spdlog::sinks::basic_file_sink_mt>(file_path);
}

MediaLibLoggerSetup::MediaLibLoggerSetup(spdlog::level::level_enum console_level, spdlog::level::level_enum file_level, spdlog::level::level_enum flush_level) : m_console_sink(make_shared_nothrow<spdlog::sinks::stderr_color_sink_mt>()),
                                                                                                                                                                 m_main_log_file_sink(create_file_sink(get_main_log_path(), MEDIALIB_LOGGER_FILENAME, true)),
                                                                                                                                                                 m_local_log_file_sink(create_file_sink(get_log_path(MEDIALIB_LOGGER_PATH_ENV_VAR), MEDIALIB_LOGGER_FILENAME, true))
{
    ...
    m_medialib_logger = make_shared_nothrow<spdlog::logger>(MEDIALIB_NAME, sink_list.begin(), sink_list.end());
    if (nullptr == m_medialib_logger)
    {
        std::cerr << "Allocating memory on heap for MediaLib logger has failed! Please check if this host has enough memory. Writing to log will result in a SEGFAULT!" << std::endl;
        return;
    }
...
bool MediaLibLoggerSetup::should_flush_every_print(const std::string &flush_every_print_env_var)
{
                                     ...
                                     spdlog::level::level_enum flush_level)
{
    ...
    bool flush_every_print = should_flush_every_print(MEDIALIB_LOGGER_FLUSH_EVERY_PRINT_ENV_VAR);
    if (flush_every_print)
    {
        m_medialib_logger->flush_on(spdlog::level::debug);
        std::cerr << "MediaLib warning: Flushing log file on every print. May reduce MediaLib performance!" << std::endl;
    }
    else
    {
...
COMPAT__INITIALIZER(libmedialib_initialize_logger) // this is a macro that calls the function below on library load (before program starts)
{
    // Init logger
    const auto log_level_file_c_str = std::getenv(MEDIALIB_LOGGER_LEVEL_ENV_VAR);
    const auto log_level_console_c_str = std::getenv(MEDIALIB_LOGGER_CONSOLE_ENV_VAR);

    auto spdlog_file_level = get_level(log_level_file_c_str, spdlog::level::level_enum::info);
    auto spdlog_console_level = get_level(log_level_console_c_str, spdlog::level::level_enum::err);
