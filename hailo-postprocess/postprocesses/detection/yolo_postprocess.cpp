  ...
  public:
          ...
          _tensors(roi->get_tensors())
    {
        if (_tensors.size() > 0)
        {
            ...
            {
                _layers.push_back(std::make_shared<Yolov5OL>(_tensors[i], params->anchors_vec[i], sigmoid,
                                                             params->label_offset, _tensors[i]->is_uint16()));
            }
  ...
  public:
          ...
          _tensors(roi->get_tensors())
    {
        if (_tensors.size() > 0)
        {
            ...
            for (std::size_t i = 0; i < _tensors.size(); i++)
            {
                _layers.push_back(std::make_shared<Yolov3OL>(_tensors[i], params->anchors_vec[i], sigmoid,
                                                             params->label_offset, _tensors[i]->is_uint16()));
            }
        }
        params->check_params_logic(get_num_classes());
  ...
  public:
          ...
          _tensors(roi->get_tensors())
    {
        if (_tensors.size() > 0)
        {
            ...
            for (std::size_t i = 0; i < _tensors.size(); i++)
            {
                _layers.push_back(std::make_shared<TinyYolov4OL>(_tensors[i], params->anchors_vec[i], sigmoid,
                                                                 params->label_offset, _tensors[i]->is_uint16()));
            }
        }
        params->check_params_logic(get_num_classes());
        ...
        : YoloPost(params->labels, params->detection_threshold, params->iou_threshold, params->max_boxes), _roi(roi)
    {
        if (_roi->has_tensors())
        {
            ...
            _layers.push_back(std::make_shared<Yolov4OL>(
                ...
                _roi->get_tensor("yolov4_leaky/conv110_centers")->is_uint16()));
            ...
            _layers.push_back(std::make_shared<Yolov4OL>(
                ...
                _roi->get_tensor("yolov4_leaky/conv103_centers")->is_uint16()));

            _layers.push_back(std::make_shared<Yolov4OL>(
                _roi->get_tensor("yolov4_leaky/conv95_centers"), _roi->get_tensor("yolov4_leaky/conv95_scales"),
                _roi->get_tensor("yolov4_leaky/conv95_obj"), _roi->get_tensor("yolov4_leaky/conv95_probs"), anchors[2],
                params->label_offset, sigmoid, _roi->get_tensor("yolov4_leaky/conv95_centers")->is_uint16()));
  ...
  public:
        ...
        : YoloPost(params->labels, params->detection_threshold, params->iou_threshold, params->max_boxes), _roi(roi)
    {
        if (_roi->has_tensors())
        {
            ...
            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_l_leaky/conv130"),
                                                        _roi->get_tensor("yolox_l_leaky/conv131"),
                                                        _roi->get_tensor("yolox_l_leaky/conv129"), params->label_offset,
                                                        _roi->get_tensor("yolox_l_leaky/conv130")->is_uint16()));

            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_l_leaky/conv113"),
                                                        _roi->get_tensor("yolox_l_leaky/conv114"),
                                                        _roi->get_tensor("yolox_l_leaky/conv112"), params->label_offset,
                                                        _roi->get_tensor("yolox_l_leaky/conv113")->is_uint16()));

            _layers.push_back(std::make_shared<YoloXOL>(_roi->get_tensor("yolox_l_leaky/conv95"),
                                                        _roi->get_tensor("yolox_l_leaky/conv96"),
                                                        _roi->get_tensor("yolox_l_leaky/conv94"), params->label_offset,
                                                        _roi->get_tensor("yolox_l_leaky/conv95")->is_uint16()));
            params->check_params_logic(get_num_classes());
        }
    };
    ~YoloX() = default;
