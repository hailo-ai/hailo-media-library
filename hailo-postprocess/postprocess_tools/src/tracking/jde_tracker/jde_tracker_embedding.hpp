 ...
 */
inline void JDETracker::embedding_distance(std::vector<STrack *> &tracks, std::vector<STrack> &detections,
 ...
 */
inline void JDETracker::fuse_motion(std::vector<std::vector<float>> &cost_matrix, std::vector<STrack *> &tracks,
                                    std::vector<STrack> &detections, float lambda_ = 0.98)
{
    ...
    for (uint i = 0; i < tracks.size(); i++)
    {
        xt::xarray<float, xt::layout_type::row_major> gating_distance =
            m_kalman_filter.gating_distance(tracks[i]->m_mean, tracks[i]->m_covariance, measurements);
