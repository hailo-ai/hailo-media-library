...
inline HailoMatrixPtr create_matrix_ptr(xt::xarray<float> &xmatrix)
{
    ...
    return std::make_shared<HailoMatrix>(std::move(data), xmatrix.shape(0), xmatrix.shape(1), xmatrix.shape(2));
}
...
{
    xt::xarray<float> xmatrix = xt::adapt(hailo_matrix->get_data(), hailo_matrix->shape());
    dump_matrix_to_file(filename, xmatrix);
}

inline void add_landmarks_to_detection(HailoDetection &detection, std::string landmarks_type,
                                       xt::xarray<float> landmarks, float threshold = 1.0f,
