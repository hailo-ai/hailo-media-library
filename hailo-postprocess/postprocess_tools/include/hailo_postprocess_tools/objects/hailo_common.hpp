...
inline void add_classification(HailoROIPtr roi, std::string type, std::string label, float confidence,
                               int class_id = NULL_CLASS_ID)
{
    add_object(roi, std::make_shared<HailoClassification>(type, class_id, label, confidence));
}

inline HailoDetectionPtr add_detection(HailoROIPtr roi, HailoBBox bbox, std::string label, float confidence,
                                       int class_id = NULL_CLASS_ID)
{
...
inline void add_segmentations(HailoROIPtr roi, std::vector<HailoSegmentation> segmentations)
{
    for (auto seg : segmentations)
    {
        add_object(roi, std::make_shared<HailoSegmentation>(seg));
    }
}
...
inline std::vector<HailoSegmentationPtr> get_hailo_segmentations(HailoROIPtr roi)
{
    std::vector<HailoObjectPtr> objects = roi->get_objects_typed(HAILO_SEGMENTATION);
    std::vector<HailoSegmentationPtr> segmentations;

    for (auto obj : objects)
    {
        segmentations.emplace_back(std::dynamic_pointer_cast<HailoSegmentation>(obj));
    }
...
inline HailoPoint fixate_point_with_bbox(HailoROIPtr hailo_roi, HailoPoint &point, const HailoBBox &fixate_bbox)
{
    ...
    return HailoPoint((xmin - fixate_bbox.xmin()) / fixate_bbox.width(),
                      (ymin - fixate_bbox.ymin()) / fixate_bbox.height());
}
