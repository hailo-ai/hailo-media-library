...
LdcMeshContext::LdcMeshContext()
    : m_last_eis_update_time(std::time(nullptr)), m_v4l2_ctrl_manager(std::make_shared<v4l2::v4l2ControlManager>())
{
}
...
media_library_return LdcMeshContext::read_vsm_config()
{
    ...
    ConfigParser config_parser(ConfigSchema::CONFIG_SCHEMA_VSM);

    // convert string to struct
    return config_parser.config_string_to_struct<vsm_config_t>(vsm_string, m_vsm_config);
}
...
media_library_return LdcMeshContext::handle_frame(HailoMediaLibraryBufferPtr input_frame)
{
    const ldc_config_t ldc_configs = input_frame->get_attached_profile()->to_frontend_config().ldc_config;
    if (ldc_configs == m_ldc_configs)
    {
        return MEDIA_LIBRARY_SUCCESS;
    }
