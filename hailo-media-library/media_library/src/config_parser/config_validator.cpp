...
{
    LOGGER__MODULE__TRACE(MODULE_NAME, "Hashing {} incrementally", j.dump());
    try
    {
        if (j.is_object())
        {
...
std::string ConfigValidator::get_current_timestamp()
{
...
tl::expected<nlohmann::json, media_library_return> ConfigValidator::create_backup_meta_data(
    const nlohmann::json &content_json)
{
    ConfigValidator validator;
    std::string arch;
    media_library_return result = validator.get_architecture(arch);
    if (result != MEDIA_LIBRARY_SUCCESS)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to get architecture for backup metadata");
        return tl::unexpected(result);
    }

    std::string hash;
    result = validator.calculate_json_hash(hash, content_json);
    if (result != MEDIA_LIBRARY_SUCCESS)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to calculate hash for backup metadata");
        return tl::unexpected(result);
    }

    std::string timestamp = get_current_timestamp();

    return nlohmann::json{{"architecture", arch},
                          {"content_hash", hash},
