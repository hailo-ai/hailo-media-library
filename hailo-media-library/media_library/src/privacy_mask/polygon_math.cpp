...
media_library_return write_polygons_to_privacy_mask_data(
    const std::vector<polygon> &polygons, const size_t frame_width, const size_t frame_height,
    privacy_mask_types::static_privacy_mask_data_t &privacy_mask_data)
{
    if (polygons.size() > MAX_NUM_OF_STATIC_PRIVACY_MASKS)
    {
        LOGGER__MODULE__ERROR(
            MODULE_NAME, "Number of polygons ({}) exceeds the maximum number of allowed ROIs ({}) in privacy mask data",
            polygons.size(), MAX_NUM_OF_STATIC_PRIVACY_MASKS);
        return media_library_return::MEDIA_LIBRARY_ERROR;
    }
    ...
    uint i = 0;
    privacy_mask_data.rois.resize(polygons.size());
    ...
    privacy_mask_data.rois.resize(i);
    privacy_mask_data.rois.shrink_to_fit();

    if (privacy_mask_data.bitmask->buffer_data->planes[0].bytesused != packaged_array_size)
    {
        LOGGER__MODULE__ERROR(
            MODULE_NAME,
            "Failed to fill polygon - privacy mask buffer size {} is not equal to the packaged array size {}",
            privacy_mask_data.bitmask->buffer_data->planes[0].bytesused, packaged_array_size);
        return media_library_return::MEDIA_LIBRARY_ERROR;
    }
