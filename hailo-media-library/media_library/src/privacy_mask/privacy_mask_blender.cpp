    ...
    const config_encoded_output_stream_t &encoded_output_streams_config, HailoMediaLibraryBufferPtr input_buffer)
{
    ...
    if (md.m_detections)
        privacy_mask::dynamic::append_rois_from_buffer_detections(*md.m_detections, masked_labels, dilation_size,
                                                                  ...
                                                                  get_or_alloc_constant_overflow_mask());
...
std::uint8_t *PrivacyMask::get_or_alloc_constant_overflow_mask()
{
    if (m_overflow_mask_buffer)
    {
        return reinterpret_cast<std::uint8_t *>(m_overflow_mask_buffer->get_plane_ptr(0));
    }

    m_overflow_mask_pool = std::make_shared<MediaLibraryBufferPool>(
        MASK_SIZE, MASK_SIZE, HAILO_FORMAT_GRAY8, 1, HAILO_MEMORY_TYPE_DMABUF, "dpm_overflow_constant_mask");
    if (m_overflow_mask_pool->init() != MEDIA_LIBRARY_SUCCESS)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to init overflow mask buffer pool");
        m_overflow_mask_pool.reset();
        return nullptr;
    }
    m_overflow_mask_buffer = std::make_shared<hailo_media_library_buffer>();
    if (m_overflow_mask_pool->acquire_buffer(m_overflow_mask_buffer) != MEDIA_LIBRARY_SUCCESS)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to acquire overflow mask buffer");
        m_overflow_mask_buffer.reset();
        m_overflow_mask_pool.reset();
        return nullptr;
    }
    auto *data = reinterpret_cast<std::uint8_t *>(m_overflow_mask_buffer->get_plane_ptr(0));
    std::memset(data, 1, MASK_SIZE * MASK_SIZE);
    return data;
}
