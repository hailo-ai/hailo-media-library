                                ...
                                int scheduler_timeout_in_ms, int num_exp)
{
    ...
    // output order
    m_infer_model->output(m_tensors_info.m_output_stitched_tensor_name)->set_format_order(HAILO_FORMAT_ORDER_NHCW);
...
int HailortAsyncStitching::process(HailoMediaLibraryBufferPtr input_buffers_fds, int awb_buffer,
                                   HailoMediaLibraryBufferPtr output_buffer, std::shared_ptr<void> stitch_context)
{
    if (set_input_buffers(input_buffers_fds, awb_buffer) != HAILO_STITCH_SUCCESS)
    {
...
int HailortAsyncStitching::set_input_buffer(int input_buffer_fd, const std::string &tensor_name)
{
    auto size = m_infer_model->input(tensor_name)->get_frame_size();
    auto status = m_bindings.input(tensor_name)->set_dma_buffer({.fd = input_buffer_fd, .size = size});
...
int HailortAsyncStitching::set_input_buffers(HailoMediaLibraryBufferPtr input_buffer, int awb_buffer)
{
    if (set_input_buffer(input_buffer->get_plane_fd(0), m_tensors_info.m_input_lef_tensor_name) != HAILO_STITCH_SUCCESS)
    {
        std::cerr << "cant set lef input buffer" << std::endl;
        return HAILO_STITCH_ERROR;
    }

    if (set_input_buffer(input_buffer->get_plane_fd(1), m_tensors_info.m_input_sef1_tensor_name) !=
        HAILO_STITCH_SUCCESS)
    {
    ...
    if (m_num_exp == 3)
    {
        if (set_input_buffer(input_buffer->get_plane_fd(2), m_tensors_info.m_input_sef2_tensor_name) !=
            HAILO_STITCH_SUCCESS)
        {
...
int HailortAsyncStitching::set_output_buffer(int output_buffer_fd, std::string tensor_name)
{
    auto size = m_infer_model->output(tensor_name)->get_frame_size();
    auto status = m_bindings.output(tensor_name)->set_dma_buffer({.fd = output_buffer_fd, .size = size});
...
int HailortAsyncStitching::set_output_buffers(HailoMediaLibraryBufferPtr output_buffer)
{
    if (set_output_buffer(output_buffer->get_plane_fd(0), m_tensors_info.m_output_stitched_tensor_name) !=
        HAILO_STITCH_SUCCESS)
    {
        return HAILO_STITCH_ERROR;
    }
