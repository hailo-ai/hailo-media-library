...
std::optional<gyro_sample_t> GyroDevice::get_last_sample()
{
    if (m_vector_samples->empty())
    {
        return std::nullopt;
    }
    return m_vector_samples->peek_last();
}
...
#define GET_GYRO_SAMPLES_BY_THRESHOLD_EXTRA_SAMPLES (1)

tl::expected<std::vector<gyro_sample_t>, gyro_status_t> GyroDevice::get_gyro_samples_by_threshold(
    uint64_t threshold_timestamp)
{
    int extra_samples = 0;
    auto samples = m_vector_samples->dequeue_many([&threshold_timestamp, &extra_samples](const gyro_sample_t &sample) {
        if (sample.timestamp_ns <= threshold_timestamp)
        {
            if (extra_samples == 0)
            {
                // assuming timestamp is monotonic increasing, set this value only if a sample is found before threshold
                extra_samples = GET_GYRO_SAMPLES_BY_THRESHOLD_EXTRA_SAMPLES;
            }
            return true;
        }
        if (extra_samples-- > 0)
