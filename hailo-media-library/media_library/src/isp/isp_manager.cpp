...
bool IspManager::switch_mode(const ModeDescriptor &desc, const frontend_config_t &frontend_config)
{
    ...
    {
        LOGGER__MODULE__INFO(MODULE_NAME, "Using fast denoise switch path to mode {}", to_string(desc.target_mode));
    }
...
void IspManager::pull_raw_video_buffers_loop()
{
    while (wait_for_raw_video_source() && m_loop_running)
    {
        ...
        if (!get_raw_buf_success)
        {
            ...
            {
                LOGGER__MODULE__DEBUG(MODULE_NAME, "Raw buffer dequeue interrupted by stream stop, exiting loop");
            }
            continue;
        }
...
void IspManager::stop()
{
    ...
    // Stop the capture devices before joining the loop thread. The loop thread
    // blocks in m_raw_capture_device->get_buffer() (v4l2 dequeue); without
    // streamoff the dequeue can wait indefinitely and the join below hangs.
    if (m_raw_capture_device)
    {
