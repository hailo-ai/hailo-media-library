...
std::optional<std::string> SensorRegistry::get_hdr_capture_path(size_t sensor_index)
{
    if (sensor_index >= sensor_config::sensor_index_to_hdr_capture.size())
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Unsupported sensor index: {}", sensor_index);
        return std::nullopt;
    }
    return sensor_config::sensor_index_to_hdr_capture[sensor_index];
}
