...
ioctl_result_t xioctl(int fd, unsigned long request, void *arg)
{
    ...
    do
    {
        ...
        {
            last_errno = errno;
            LOGGER__MODULE__WARN(MODULE_NAME, "ioctl failed: {}", strerror(last_errno));
        }
    } while (--tries > 0 && r == -1 && EINTR == last_errno);
