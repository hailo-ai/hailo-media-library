...
ioctl_result_t xioctl(int fd, unsigned long request, void *arg);
...
class v4l2ControlManager
{
  ...
  public:
    ...
    bool ctrl_set_retry_on_eagain(CtrlEnum id, T val, size_t max_tries, std::chrono::milliseconds wait_time)
    {
        ...
        ioctl_result_t result;
        for (size_t current_try = 0; current_try < max_tries; ++current_try)
        {
            result = xioctl(*shared_fd, ctrl_id.value(), &val);
            if (result.return_errno != EAGAIN)
            {
                break;
            }
    ...
    template <class T, class CtrlEnum> bool ctrl_set(CtrlEnum id, T val)
    {
        ...
        if (!xioctl(*shared_fd, ctrl_id.value(), &val).success())
        {
            return false;
        }
    ...
    template <class T, class CtrlEnum> bool ext_ctrl_set(CtrlEnum id, T val)
    {
        ...
        if (!xioctl(*shared_fd, VIDIOC_QUERY_EXT_CTRL, &qctrl).success())
        {
            return false;
        }
        ...
        if (!xioctl(*shared_fd, VIDIOC_S_EXT_CTRLS, &ctrls).success())
        {
            return false;
        }
        ...
        -> std::optional<std::conditional_t<std::is_pointer<T>::value, std::remove_pointer_t<T>, T>>
    {
        ...
        if (!xioctl(*shared_fd, VIDIOC_G_EXT_CTRLS, &ctrls).success())
        {
            return std::nullopt;
        }
        if constexpr (std::is_pointer_v<T>)
