...
#include "server_session_registry.hpp"
...
{
  public:
    explicit MediaLibraryServiceImpl(bool persistent_config_mode = false)
        : m_user_profile(std::nullopt), m_persistent_config_mode(persistent_config_mode),
  ...
  private:
    ...
    */
    ...
    {
        std::lock_guard<std::mutex> init_lock(m_initialize_mutex);

        if (m_initialized)
        {
            // In persistent-config mode, a client Initialize() must not override
            // the persisted configuration. Keep the current pipeline as-is and
            // only wire up the client's frontend→encoder subscriptions so its
            // streams work against the already-running configuration.
            if (m_persistent_config_mode && !from_persisted_config)
            {
                LOGGER__MODULE__INFO(
                    LoggerType::Service,
                    "Persistent config mode: ignoring client config_string, keeping persisted configuration");
                return setup_output_and_subscriptions();
            }
...
#define FINISH_WITH_STATUS(reactor, reply, ret, error_msg)                                                             \
                                   ...
                                   MedialibraryStatusReply *reply) override
    {
        ...
        if (!m_session_registry.try_acquire(request->client_id()))
        {
                                 ...
                                 MedialibraryStatusReply *reply) override
    {
        ...
        {
            m_session_registry.release(request->client_id());
        }
        else
        {
            m_session_registry.release_active();
        }
                                  ...
                                  MedialibraryStatusReply *reply) override
    {
        ...
        ServerUnaryReactor *reactor = context->DefaultReactor();
        if (m_session_registry.keepalive(request->client_id()))
        {
            reply->set_status(convert_to_proto_status(media_library_return::MEDIA_LIBRARY_SUCCESS));
        }
        else
    ...
    ServerUnaryReactor *Reconnect(CallbackServerContext *context,
                                  const media_library_service::ReconnectRequest *request,
                                  MedialibraryStatusReply *reply) override
    {
        SERVICE_TRACE_RPC("Reconnect");
        LOGGER__MODULE__INFO(LoggerType::Service, "Reconnect RPC called, client_id={}", request->client_id());
        ServerUnaryReactor *reactor = context->DefaultReactor();
        if (m_session_registry.try_acquire(request->client_id()))
        {
            reply->set_status(convert_to_proto_status(media_library_return::MEDIA_LIBRARY_SUCCESS));
        ...
        {
            reply->set_status(convert_to_proto_status(media_library_return::MEDIA_LIBRARY_ERROR));
            reply->set_error_message("Session held by another client");
        }
        reactor->Finish(Status::OK);
        return reactor;
    }
    ...
    std::mutex m_initialize_mutex; // serializes self-init and client Initialize / reconfigure
    MediaLibraryPtr m_media_lib;
    std::map<output_stream_id_t, cloexec::ofstream> m_output_files;
    std::optional<config_profile_t> m_user_profile;
    std::mutex m_profile_mutex; // protects m_user_profile
    bool m_persistent_config_mode{false};
    ServerSessionRegistry m_session_registry;
...
int main()
{
    ...
    bool persistent_config_mode = (std::getenv(MEDIALIB_SERVICE_PERSISTENT_CONFIG_MODE_ENV_VAR) != nullptr);
    if (persistent_config_mode)
    {
        LOGGER__MODULE__INFO(LoggerType::Service,
                             "Persistent config mode enabled ({} is set): client Initialize() will not override "
                             "persisted configuration",
                             MEDIALIB_SERVICE_PERSISTENT_CONFIG_MODE_ENV_VAR);
    }

    MediaLibraryServiceImpl service(persistent_config_mode);
