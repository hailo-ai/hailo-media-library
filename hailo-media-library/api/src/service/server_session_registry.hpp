...
/**
 * @brief Server-side registry of the single active client session.
 *
 * Lives in the media library service process. Tracks which (if any) client
 * currently holds the exclusive session, evicts a session whose keepalive has
 * lapsed, and notifies subscribed observers when a client connects or disconnects.
 *
 * Counterpart on the client side is ClientSessionKeeper, which drives
 * Initialize / KeepAlive / Reconnect against this registry over gRPC.
 *
