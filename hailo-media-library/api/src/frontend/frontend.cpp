...
tl::expected<bool, media_library_return> MediaLibraryFrontend::get_freeze()
{
    return m_impl->get_freeze();
}
...
tl::expected<frontend_config_t, media_library_return> MediaLibraryFrontend::Impl::get_config()
{
    ...
    {
        return m_config_manager_interactor->get_frontend_config();
    }
    ...
    g_object_get(frontendbinsrc.as_g_object(), "config-manager-interactor",
                 &config_manager_interactor_for_dummy_profile, NULL);
    ...
    return config_manager_interactor_for_dummy_profile->get_frontend_config();
}
...
media_library_return MediaLibraryFrontend::Impl::start()
{
    ...
    LOGGER__MODULE__DEBUG(MODULE_NAME, "Starting frontend gst pipeline");
    ...
    LOGGER__MODULE__TRACE(MODULE_NAME, "Frontend pipeline started");
    return MEDIA_LIBRARY_SUCCESS;
}
...
media_library_return MediaLibraryFrontend::Impl::stop()
{
    ...
    LOGGER__MODULE__DEBUG(MODULE_NAME, "Flushing frontend gst pipeline before stopping");
    flush_pipeline();

    LOGGER__MODULE__INFO(MODULE_NAME, "Stopping frontend gst pipeline, Sending EOS event");
    gboolean ret = gst_element_send_event(m_pipeline, gst_event_new_eos());
    if (!ret)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to stop pipeline by sending EOS event");
        return MEDIA_LIBRARY_ERROR;
    }
    ...
    if (passed_timeout)
    {
        ...
        LOGGER__MODULE__DEBUG(MODULE_NAME, "MediaLibraryFrontend::Impl::stop() Pipeline state set to NULL");
        g_main_loop_quit(m_main_loop);
        LOGGER__MODULE__DEBUG(MODULE_NAME, "MediaLibraryFrontend::Impl::stop() Main loop quit");
    }
    if (0 != m_bus_watch_id)
    {
    ...
    {
        LOGGER__MODULE__DEBUG(MODULE_NAME, "MediaLibraryFrontend::Impl::stop() Waiting for main loop thread to join");
        m_main_loop_thread->join();
        LOGGER__MODULE__DEBUG(MODULE_NAME, "MediaLibraryFrontend::Impl::stop() Main loop thread joined");
    }

    LOGGER__MODULE__TRACE(MODULE_NAME, "Frontend pipeline stopped");
    return MEDIA_LIBRARY_SUCCESS;
}
...
media_library_return MediaLibraryFrontend::Impl::pause_pipeline()
{
    ...
    LOGGER__MODULE__TRACE(MODULE_NAME, "Frontend pipeline paused");
    return MEDIA_LIBRARY_SUCCESS;
}
...
media_library_return MediaLibraryFrontend::Impl::unpause_pipeline()
{
    ...
    LOGGER__MODULE__DEBUG(MODULE_NAME, "Unpausing frontend gst pipeline");
    GstState state;
    ...
    LOGGER__MODULE__TRACE(MODULE_NAME, "Frontend pipeline unpaused");
    return MEDIA_LIBRARY_SUCCESS;
}
...
media_library_return MediaLibraryFrontend::Impl::handle_outputs_change(const frontend_config_t &new_cfg)
{
    ...
    for (auto &resolution : new_outs.value())
    {
        if (output_configs_diff.added.end() == std::find_if(output_configs_diff.added.begin(),
                                                            output_configs_diff.added.end(),
                                                            [&](const std::pair<int, frontend_output_stream_t> &added) {
                                                                return added.second.id == resolution.id;
                                                            }))
        {
            continue; // not an added stream
        }
        m_output_streams.push_back(resolution);
...
{
    LOGGER__MODULE__INFO(MODULE_NAME, "Removing output stream id '{}'", removed_output.id);
    ...
    m_output_stream_by_srcpad_name.erase(removed_output.srcpad_name);
...
{
    LOGGER__MODULE__INFO(MODULE_NAME, "Adding output stream id '{}'", stream.id);
    auto frontendbinsrc = glib_cpp::ptrs::get_bin_by_name(m_pipeline, "frontend");
    if (frontendbinsrc == nullptr)
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to get frontend element");
                                                               ...
                                                               frontend_output_stream_t &stream)
{
    ...
    m_output_stream_by_srcpad_name[stream.srcpad_name] = stream.id;

    // Create appsink
    GstElementPtr appsink = gst_element_factory_make("appsink", stream.srcpad_name.c_str());
    if (!appsink)
    {
                                                          ...
                                                          std::vector<frontend_output_stream_t> &output_streams)
{
    ...
    case frontend_src_element_t::APPSRC:
        ...
        {
            g_object_set(frontend.as_g_object(), "config-manager-interactor", m_config_manager_interactor, NULL);
        }
...
media_library_return MediaLibraryFrontend::Impl::set_freeze(bool freeze)
{
    ...
    g_object_set(frontend.as_g_object(), "freeze", (gboolean)(freeze ? TRUE : FALSE), NULL);
    return MEDIA_LIBRARY_SUCCESS;
}

tl::expected<bool, media_library_return> MediaLibraryFrontend::Impl::get_freeze()
{
    GstElementPtr frontend = glib_cpp::ptrs::get_bin_by_name(m_pipeline, "frontend");
    if (frontend == nullptr)
    {
    ...
    g_object_get(frontend.as_g_object(), "freeze", &freeze, NULL);
    return freeze == TRUE;
}

media_library_return MediaLibraryFrontend::Impl::flush_pipeline()
{
    if (m_pipeline == nullptr)
    {
    ...
    if (!gst_element_send_event(m_pipeline.get(), flush_start))
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to send flush start event");
        return MEDIA_LIBRARY_ERROR;
    }

    if (!gst_element_send_event(m_pipeline.get(), flush_stop))
    {
        LOGGER__MODULE__ERROR(MODULE_NAME, "Failed to send flush stop event");
        return MEDIA_LIBRARY_ERROR;
    }

    LOGGER__MODULE__DEBUG(MODULE_NAME, "Pipeline flushed successfully");
    return MEDIA_LIBRARY_SUCCESS;
}
...
void MediaLibraryFrontend::set_config_manager_interactor(const ConfigManagerInteractor &config_manager_interactor)
{
    m_impl->set_config_manager_interactor(config_manager_interactor);
}

void MediaLibraryFrontend::Impl::set_config_manager_interactor(const ConfigManagerInteractor &config_manager_interactor)
{
