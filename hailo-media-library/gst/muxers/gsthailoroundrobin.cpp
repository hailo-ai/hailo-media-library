...
static void gst_hailo_round_robin_pad_class_init([[maybe_unused]] GstHailoRoundRobinPadClass *klass)
{
}
...
static gboolean gst_hailo_round_robin_condvar_contains_pad(GstHailoRoundRobin *hailo_round_robin, guint pad_num,
                                                           gboolean blocking);
...
static gboolean forward_events([[maybe_unused]] GstPad *pad, GstEvent **event, gpointer user_data)
{
    gboolean res = TRUE;
...
static GstPad *gst_hailo_round_robin_request_new_pad(GstElement *element, GstPadTemplate *templ,
                                                     [[maybe_unused]] const gchar *name,
                                                     [[maybe_unused]] const GstCaps *caps)
{
                                                           ...
                                                           gboolean blocking)
{
    if (blocking)
    {
        return (hailo_round_robin->condition_vars_blocking.size() >= pad_num &&
                hailo_round_robin->condition_vars_blocking[pad_num] != NULL);
    }

    return (hailo_round_robin->condition_vars_non_blocking.size() >= pad_num &&
...
static void gst_hailo_round_robin_release_pad(GstElement *element, GstPad *pad)
{
    ...
    {
        GST_DEBUG_OBJECT(hailo_round_robin, "Notifying condition_vars_blocking pad %zu that it is released",
                         get_pad_num(pad));
    ...
    {
        GST_DEBUG_OBJECT(hailo_round_robin, "Notifying condition_vars_non_blocking pad %zu that it is released",
                         get_pad_num(pad));
...
static gboolean gst_hailo_round_robin_sink_event(GstPad *pad, GstObject *parent, GstEvent *event)
{
    ...
    if (GST_EVENT_IS_STICKY(event))
    {
        ...
        if (GST_EVENT_TYPE(event) == GST_EVENT_EOS)
        {
            GST_DEBUG_OBJECT(hailo_round_robin,
                             "Received EOS event on pad %zu - Locking the pad and notifying all condition variables",
                             pad_num);
    ...
    else if (GST_EVENT_TYPE(event) == GST_EVENT_FLUSH_STOP)
    {
        ...
        GST_PAD_STREAM_LOCK(hailo_round_robin->srcpad);
        GST_DEBUG_OBJECT(hailo_round_robin,
                         "Received FLUSH_STOP event on pad %zu - Locking the pad and notifying all condition variables",
                         pad_num);
        GST_OBJECT_LOCK(hailo_round_robin);
        fpad->got_eos = FALSE;
        GST_OBJECT_UNLOCK(hailo_round_robin);
