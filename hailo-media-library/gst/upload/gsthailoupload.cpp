...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_hailo_upload_debug, "hailoupload", 0, "Hailo Upload");
...
static GstFlowReturn gst_hailo_upload_transform(GstBaseTransform *base_transform, GstBuffer *inbuf, GstBuffer *outbuf);
...
static void gst_hailo_upload_class_init(GstHailoUploadClass *klass)
{
    ...
    gst_element_class_set_static_metadata(GST_ELEMENT_CLASS(klass), "Hailo Upload", "Hailo/Tools",
                                          "Uploads a buffer to the Hailo15 DSP memory", "hailo.ai <contact@hailo.ai>");
    ...
    g_object_class_install_property(
        object_class, PROP_USE_GPDMA,
        g_param_spec_boolean("use-gpdma", "Use GPDMA", "Use GPDMA for memory copy", FALSE,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
}

static void gst_hailo_upload_init(GstHailoUpload *self)
{
...
static GstFlowReturn gpdma_copy_channel(GstHailoUpload *hailoupload, void *channel_data, void *output_channel_data,
                                        size_t channel_size)
{
    ...
    GST_LOG_OBJECT(hailoupload, "virt_src_addr: %lu, virt_dst_addr: %lu, length: %lu, status: %d",
                   ioctl_data.virt_src_addr, ioctl_data.virt_dst_addr, ioctl_data.length, ioctl_data.status);
...
static GstFlowReturn gst_hailo_upload_transform(GstBaseTransform *base_transform, GstBuffer *inbuf, GstBuffer *outbuf)
{
    ...
    if (!gst_caps_is_fixed(incaps))
    {
        GST_ERROR_OBJECT(hailoupload, "Input caps are not fixed");
    }
    ...
    case GST_VIDEO_FORMAT_NV12: {
        ...
        if (hailoupload->use_gpdma)
        {
            ...
            if (gpdma_copy_channel(hailoupload, y_input_channel_data, y_output_channel_data, y_channel_size) !=
                GST_FLOW_OK)
            {
            ...
            if (gpdma_copy_channel(hailoupload, uv_input_channel_data, uv_output_channel_data, uv_channel_size) !=
                GST_FLOW_OK)
            {
        ...
        else
        {
            ...
            GST_DEBUG_OBJECT(hailoupload, "NV12 format: Copied Y channel %d bytes from input buffer to output buffer",
                             (int)y_channel_size);
            ...
            if (DmaMemoryAllocator::get_instance().dmabuf_sync_start(uv_output_channel_userptr) !=
